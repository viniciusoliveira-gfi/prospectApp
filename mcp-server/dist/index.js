    ...
    if (seq?.status === "active" && seq.started_at) {
        ...
        for (const step of steps) {
            const { data: existingScheduled } = await supabase
                ...
                .limit(1);
            if (existingScheduled?.length && existingScheduled[0].scheduled_for) {
                const stepEmails = data.filter((e) => e.sequence_step_id === step.id);
                for (const email of stepEmails) {
                    await supabase.from("emails").update({
                        send_status: "scheduled",
        ...
        if (scheduled > 0)
            scheduleNote = ` Auto-scheduled ${scheduled} emails to match existing step dates.`;
    }
...
server.tool("update_email", "Edit an email's subject, body, status, or reply state. Set replied=false to clear false reply detection.", {
    email_id: z.string(),
...
server.tool("retry_failed_emails", "Reset failed (and optionally skipped) emails back to scheduled so they can be retried. Call recalculate_campaign_schedule after to assign new send dates.", {
    campaign_id: z.string(),
    ...
    if (!emails?.length)
        return { content: [{ type: "text", text: "No failed/skipped emails found." }] };
...
server.tool("recalculate_campaign_schedule", "Recalculate schedules for ALL sequences in a campaign, distributing emails across days so no day exceeds total daily capacity. Respects send days, per-account limits, and step ordering.", {
    ...
    const step1Emails = [];
    const laterEmails = new Map(); // contact_id -> [step 2, 3, 4...]
    for (const [contactId, contactEmails] of Array.from(emailsByContact)) {
        for (const email of contactEmails) {
            if (email.step_number === 1) {
                ...
                if (!laterEmails.has(contactId))
                    laterEmails.set(contactId, []);
    ...
    function getAvailableDay(startFrom) {
    ...
    for (const email of step1Emails) {
        ...
        if (!contactStepDates.has(email.contact_id))
            contactStepDates.set(email.contact_id, new Map());
    ...
    for (const [contactId] of Array.from(laterEmails)) {
        if (contactStepDates.has(contactId))
            continue; // already scheduled step 1 above
            ...
            for (const sent of sentForContact) {
                ...
                for (const seq of sequences) {
                    ...
                    if (step) {
                        ...
                        if (date)
                            stepMap.set(step.step_number, date);
                    }
                }
            }
            if (stepMap.size)
                contactStepDates.set(contactId, stepMap);
        }
    }
